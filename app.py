...
.hero h1 {font-size: 44px; margin-bottom: 6px;}
.hero p {font-size: 17px; color: #e5e7eb;}
.section-title {font-size: 23px; font-weight: 700; margin-top: 20px; margin-bottom: 8px;}
</style>
...
def create_data():
    ...
    data = pd.DataFrame({
        ...
        "segment": np.random.choice(["SMB", "Mid-Market", "Enterprise"], n, p=[0.48, 0.34, 0.18]),
        "industry": np.random.choice(["Logistics", "Healthcare", "Retail", "Financial Services", "Education", "SaaS"], n),
        "plan": np.random.choice(["Starter", "Growth", "Pro", "Enterprise"], n, p=[0.28, 0.34, 0.25, 0.13]),
        "region": np.random.choice(["North America", "Europe", "Africa", "Asia-Pacific"], n, p=[0.45, 0.22, 0.18, 0.15]),
        "tenure_months": np.random.randint(1, 49, n),
        "monthly_recurring_revenue": np.random.gamma(5, 90, n).round(2),
        "active_days_last_30": np.random.randint(0, 31, n),
        "support_tickets_last_30": np.random.poisson(2.2, n),
        "avg_response_time_hours": np.random.gamma(2.2, 4, n).round(1),
        "feature_adoption_score": np.random.beta(2.2, 2.4, n).round(3),
        "nps_score": np.random.randint(0, 11, n),
        "billing_issues_last_90": np.random.poisson(0.5, n),
        "training_completed": np.random.choice([0, 1], n, p=[0.38, 0.62]),
        "last_login_days_ago": np.random.randint(0, 61, n)
    })

    logit = (
        -2.2
        + 0.08 * data["support_tickets_last_30"]
        + 0.09 * data["avg_response_time_hours"]
        - 0.10 * data["active_days_last_30"]
        - 2.3 * data["feature_adoption_score"]
        - 0.18 * data["nps_score"]
        + 0.04 * data["last_login_days_ago"]
        + 0.35 * data["billing_issues_last_90"]
        - 0.25 * data["training_completed"]
        + np.where(data["plan"].eq("Starter"), 0.35, 0)
    )
    probability = 1 / (1 + np.exp(-logit))
    data["churned"] = np.random.binomial(1, probability)

    data.loc[data["plan"].eq("Enterprise"), "monthly_recurring_revenue"] *= 4
    data.loc[data["segment"].eq("Enterprise"), "monthly_recurring_revenue"] *= 2.5
    data["monthly_recurring_revenue"] = data["monthly_recurring_revenue"].round(2)
    return data
...
numeric_features = [
    "tenure_months", "monthly_recurring_revenue", "active_days_last_30",
    "support_tickets_last_30", "avg_response_time_hours", "feature_adoption_score",
    "nps_score", "billing_issues_last_90", "training_completed", "last_login_days_ago"
]
...
data["predicted_churn_probability"] = model.predict_proba(X)[:, 1]
...
with st.sidebar:
    ...
    st.divider()
    st.caption("Built with Python, Streamlit, Scikit-learn, Plotly, and Pandas.")
...
if total_customers:
    threshold = filtered["monthly_recurring_revenue"].quantile(0.75)
    high_value_at_risk = filtered[(filtered["risk_level"] == "High") & (filtered["monthly_recurring_revenue"] >= threshold)]
else:
...
with left:
    ...
    segment_risk = filtered.groupby(["segment", "risk_level"]).size().reset_index(name="customers")
    fig = px.bar(segment_risk, x="segment", y="customers", color="risk_level", barmode="group")
...
with left2:
    ...
    plan_revenue = filtered.groupby("plan")["revenue_at_risk"].sum().sort_values(ascending=False).reset_index()
    fig3 = px.bar(plan_revenue, x="plan", y="revenue_at_risk")
...
review = review[[
    "customer_id", "segment", "industry", "plan", "monthly_recurring_revenue",
    "predicted_churn_probability", "risk_level", "active_days_last_30",
    "feature_adoption_score", "nps_score", "support_tickets_last_30",
    "revenue_at_risk", "risk_explanation"
]]
...
with p1:
    ...
    st.write("Customer success and product teams often know churn is happening, but lack a clear way to prioritize which customers need intervention first.")

with p2:
    st.subheader("Product Solution")
    st.write("ChurnGuard predicts churn probability, explains major risk drivers, and connects retention risk to revenue impact.")

with p3:
    st.subheader("Success Metrics")
    st.write("Reduced churn, increased retention saves, improved outreach prioritization, higher feature adoption, and faster customer recovery.")
